...
var VXDOOM = (function () {
    ...
    var _f7 = '', _f8 = '', _f9 = '', _f9b = '', _f11 = '', _f11b = '', _f14b = '', _f16b = '', _f22b = '', _f28b = '', _f42b = '';
    var C = {
        bg: "#0c0606", ship: "#ff2828", shipC: "#ff5050", shipG: "#ff2233", bul: "#ff4444",
    ...
    function mkSp() { return { x: 0, y: 0, vx: 0, vy: 0, life: 0, ml: 0, r: 0, cr: 0, cg: 0, cb: 0, col: '', on: false, gv: 0 } }
    ...
    function initPools() {
        bullets = []; enemies = []; sparks = []; drops = []; eBullets = [];
    ...
    function fire(x, y, sp, wt) {
        var b = gp(bullets, mkBul, LIM.b); if (!b) return; var wn = wt !== undefined ? wt : weaponType; b.x = x; b.y = y; b.vx = (sp || 0) * dpr; b.pow = powerUp > 0 ? 1 : 0; b.wt = wn; b.homing = false;
    ...
    function grantWeapon() {
        var wt = ri(1, 3); weaponType = wt; weaponAmmo = ri(18, 30); snd("pickup");
    ...
    function boom(x, y, n, cr, cg, cb) { for (var i = 0; i < n; i++) { var s = gp(sparks, mkSp, LIM.s); if (!s) break; var a = rng(0, PI * 2), sp = rng(1.5, 5.5) * dpr; s.x = x; s.y = y; s.vx = cos(a) * sp; s.vy = sin(a) * sp; s.life = rng(8, 25); s.ml = 25; s.r = rng(1, 3.5) * dpr; s.cr = cr || 255; s.cg = cg || 30; s.cb = cb || 30; s.col = "rgb(" + (cr || 255) + "," + (cg || 30) + "," + (cb || 30) + ")"; s.on = true; s.gv = 0 } }
    function boomRing(x, y, n, rad, cr, cg, cb) { for (var i = 0; i < n; i++) { var s = gp(sparks, mkSp, LIM.s); if (!s) break; var a = PI * 2 * (i / n); s.x = x + cos(a) * rad * .3; s.y = y + sin(a) * rad * .3; s.vx = cos(a) * rad * .11; s.vy = sin(a) * rad * .11; s.life = 16; s.ml = 16; s.r = rng(2, 3) * dpr; s.cr = cr; s.cg = cg; s.cb = cb; s.col = "rgb(" + cr + "," + cg + "," + cb + ")"; s.on = true; s.gv = 0 } }
    ...
    function spawnEn(type, x, y) {
        var e = gp(enemies, mkEn, LIM.e); if (!e) return; e.x = x || rng(60 * dpr, W - 60 * dpr); e.y = y || rng(-80 * dpr, -20 * dpr); e.type = type || 0; e.on = true; e.ph = rng(0, PI * 2); e.rot = 0; e.ft = ri(40, 140); e.age = 0; e.hf = 0; e.edge = 0; e.tx = 0; e.ty = 0; var s = dpr, lm = 1 + level * .08;
        if (type === 0) { e.w = 32 * s; e.h = 32 * s; e.hp = 1; e.mhp = 1; e.vy = .65 * s; e.amp = 0; e.vx = 0 }
            ...
            e.w = 28 * s; e.h = 28 * s; e.hp = 2; e.mhp = 2; e.vy = 0; e.amp = 0; e.vx = 0; e.edge = 1;
    ...
    function spawnWave() {
        waveNum++; var cnt = cl(5 + F(level * 1.3), 5, 16); var tp = [0, 0, 0, 1];
        ...
        var fm = waveNum % 5; for (var i = 0; i < cnt; i++) {
            var t = tp[ri(0, tp.length - 1)], ex, ey;
    ...
    function snd(type) {
        if (!aCtx || muted) return; if (aCtx.state === "suspended") aCtx.resume(); try {
            var o = aCtx.createOscillator(), g = aCtx.createGain(), n = aCtx.currentTime; o.connect(g); g.connect(mGain);
    ...
    function updBullets() {
        for (var i = 0; i < bullets.length; i++) {
            var b = bullets[i]; if (!b.on) continue;
            ...
            for (var j = 0; j < enemies.length; j++) {
                var e = enemies[j]; if (!e.on) continue;
                ...
                if (abs(b.x - e.x) < (hw + e.w) * .44 && abs(b.y - e.y) < (hh + e.h) * .44) {
                    ...
                    if (e.hp <= 0) {
                        e.on = false; var pts = (e.type + 1) * 15; combo++; comboT = 90; kStreak++;
            ...
            if (b.on && bossOn && boss && boss.on && boss.entered) {
                if (abs(b.x - boss.x) < (boss.w * .44 + b.w) && abs(b.y - boss.y) < (boss.h * .44 + b.h)) {
                    if (b.wt !== 1) b.on = false; boss.hp -= dmg; boss.shf = 4; boom(b.x, b.y, 3, 255, 30, 80);
                    if (boss.hp <= 0) {
                        boss.on = false; bossOn = false; score += 800 + level * 150; if (onScore) onScore(score, level, lives);
    ...
    function updEnemies() {
        for (var i = 0; i < enemies.length; i++) {
            var e = enemies[i]; if (!e.on) continue; e.ph += .02; e.rot += .015; e.age++; if (e.hf > 0) e.hf--;
            if (e.type === 5) {
                var tx5 = e.tx, ty5 = e.ty;
                if (e.edge === 1) {
                    e.x = lr(e.x, tx5, .03); e.y = lr(e.y, ty5 + sin(e.ph) * 25 * dpr, .025);
                    if (abs(e.x - tx5) < 5 * dpr) { e.edge = 2; e.tx = ship ? ship.x : W / 2; e.ty = ship ? ship.y - 40 * dpr : H / 2 }
                }
                else if (e.edge === 2) {
                    e.x = lr(e.x, e.tx, .025); e.y = lr(e.y, e.ty, .025);
                ...
                if (e.ft <= 0 && ship) {
                    e.ft = mx(10, ri(45, 150) - level * 3); if (R() < .28 + level * .03) {
                        fireE(e.x, e.y + e.h / 2, ship.x, ship.y);
    ...
    function updEB() {
        for (var i = 0; i < eBullets.length; i++) {
            var b = eBullets[i]; if (!b.on) continue;
            b.x += b.vx * (slowMo > 0 ? .4 : 1); b.y += b.vy * (slowMo > 0 ? .4 : 1);
            if (b.y > H + 20 || b.y < -20 || b.x < -20 || b.x > W + 20) { b.on = false; continue }
            if (ship && ship.inv <= 0) {
                if (ship.shield > 0 && abs(b.x - ship.x) < (ship.w * .55 + b.w) && abs(b.y - ship.y) < (ship.h * .55 + b.w)) { b.on = false; boom(b.x, b.y, 3, 60, 160, 255); score += 5; continue }
                if (abs(b.x - ship.x) < (ship.w * .3 + b.w) && abs(b.y - ship.y) < (ship.h * .3 + b.w)) { b.on = false; hitShip() }
            }
        }
    }
    function updBoss() {
        if (!bossOn || !boss || !boss.on) return; if (boss.shf > 0) boss.shf--;
        ...
        if (boss.phase === 1) { boss.x = W / 2 + sin(boss.ph) * (W * .3) }
            ...
            if (boss.teleT > 180 && level > 5) { boss.teleT = 0; boss.x = rng(W * .15, W * .85); boss.y = rng(50 * dpr, H * .35); doGlitch(8) }
            else { boss.x = lr(boss.x, W / 2 + sin(boss.ph * 1.5) * (W * .4), .04); boss.y = lr(boss.y, 60 * dpr + sin(boss.ph * .9) * 50 * dpr, .04) }
        }
        if (boss.ft <= 0 && ship) {
            boss.ft = mx(6, 22 - level * 2 - boss.phase * 3); var p = boss.pat % (3 + boss.phase);
    ...
    function updDrops() {
        for (var i = 0; i < drops.length; i++) {
            var d = drops[i]; if (!d.on) continue; d.y += d.vy; d.ph += .08;
            if (d.y > H + 30) { d.on = false; continue }
            if (ship && abs(d.x - ship.x) < 24 * dpr && abs(d.y - ship.y) < 24 * dpr) {
                d.on = false; var rgb = hx(C.dr[d.type]);
    ...
    function updGlitch() {
        if (glitch) { glitchT++; if (glitchT >= glitchD) glitch = false }
    ...
    function drawShip(c) {
        if (!ship) return; if (ship.inv > 0 && tick % 5 < 2) return;
    ...
    function drawEn(c) {
        for (var i = 0; i < enemies.length; i++) {
            var e = enemies[i]; if (!e.on) continue;
    ...
    function drawBoss(c) {
        if (!bossOn || !boss || !boss.on) return; c.save(); c.translate(boss.x, boss.y);
        ...
        c.fillStyle = "#aa4455"; c.font = _f7; c.textAlign = "center"; c.fillText("PHASE " + ph, 0, -boss.h / 2 - 17 * dpr);
        c.restore()
    }
    function drawDrops(c) {
        for (var i = 0; i < drops.length; i++) {
            var d = drops[i]; if (!d.on) continue; c.fillStyle = C.dr[mn(d.type, 4)]; c.globalAlpha = .6 + sin(d.ph) * .3; c.beginPath(); c.arc(d.x, d.y, d.w / 2, 0, PI * 2); c.fill(); c.globalAlpha = .18; c.strokeStyle = C.dr[mn(d.type, 4)]; c.lineWidth = dpr; c.beginPath(); c.arc(d.x, d.y, d.w / 2 + 3 * dpr + sin(d.ph * 2) * 2 * dpr, 0, PI * 2); c.stroke(); c.globalAlpha = 1;
            if (d.type === 4) { c.fillStyle = "#ff55ff"; c.globalAlpha = .85; c.font = _f9b; c.textAlign = "center"; c.fillText("W", d.x, d.y + 3 * dpr); c.globalAlpha = 1 }
        }
    }
    function drawSparks(c) { for (var i = 0; i < sparks.length; i++) { var s = sparks[i]; if (!s.on) continue; var t = s.life / s.ml; c.globalAlpha = t; c.fillStyle = s.col; c.fillRect(s.x - s.r, s.y - s.r, s.r * 2, s.r * 2) } c.globalAlpha = 1 }
    function drawHUD(c) {
        c.font = _f16b; c.fillStyle = C.txH; c.textAlign = "left"; c.fillText(score, 12 * dpr, 24 * dpr);
        c.font = _f8; c.fillStyle = C.txD; c.fillText("HI " + best, 12 * dpr, 35 * dpr);
        c.textAlign = "right"; c.fillStyle = C.txH; c.font = _f11b; c.fillText("LV" + level, W - 12 * dpr, 24 * dpr); c.fillText("W" + waveNum, W - 12 * dpr, 36 * dpr);
        for (var i = 0; i < 5; i++) { c.fillStyle = i < lives ? "#ff3344" : "#220808"; tri(c, W - 12 * dpr - i * 15 * dpr, 46 * dpr, 9 * dpr, 9 * dpr, false); c.fill() }
        if (ship && ship.shield > 0) { c.fillStyle = "#66aaff"; c.textAlign = "center"; c.font = _f8; c.fillText("SHIELD " + F(ship.shield / 60) + "s", W / 2, H - 26 * dpr) }
        if (powerUp > 0) { c.fillStyle = C.cmb; c.textAlign = "center"; c.font = _f8; c.fillText("POWER " + F(powerUp / 60) + "s", W / 2, H - 14 * dpr) }
        if (combo > 1) { c.textAlign = "center"; c.fillStyle = combo >= 10 ? "#ffff00" : (combo >= 5 ? "#ffbb00" : C.cmb); c.globalAlpha = cl(comboT / 80, 0, 1); c.font = _f28b; c.fillText("x" + combo, W / 2, H * .35); c.globalAlpha = 1 }
        if (kStreak >= 5) { c.textAlign = "center"; c.font = _f9b; c.fillStyle = kStreak >= 10 ? "#66aaff" : "#ff8855"; c.fillText("STREAK " + kStreak, W / 2, H * .42) }
        if (warnT > 0) { c.textAlign = "center"; c.font = _f14b; c.globalAlpha = mn(1, warnT / 20); c.fillStyle = "#ff5555"; c.fillText(warnTxt, W / 2, H * .25); c.globalAlpha = 1 }
        ...
        if (paused) {
            c.fillStyle = "rgba(5,0,0,.92)"; c.fillRect(0, 0, W, H);
            ...
            c.textAlign = "center"; c.fillStyle = "#ff6666"; c.font = _f22b; c.fillText("── PAUSED ──", bx, by + 32 * dpr);
            ...
            if (sin(tick * .08) > 0) { c.fillStyle = "#ff5555"; c.font = _f9b; c.fillText("[ ESC ] RESUME", bx, by + bh - 10 * dpr) }
        }
    }
    function drawVHS(c) {
        if (glitch) {
            var n = ri(2, 6); for (var g = 0; g < n; g++) { var gy = ri(0, H), gh = ri(1, 8) * dpr, gx = ri(-25, 25) * dpr; c.fillStyle = "rgba(255,0,0," + ((.12 + R() * .14).toFixed(3)) + ")"; c.fillRect(gx, gy, W, gh); c.fillStyle = "rgba(0,255,0," + ((.06 + R() * .08).toFixed(3)) + ")"; c.fillRect(-gx, gy + gh, W, gh); if (R() < .4) { c.fillStyle = "rgba(0,0,255," + ((.05 + R() * .06).toFixed(3)) + ")"; c.fillRect(gx * .5, gy + gh * 2, W, gh * .5) } }
    ...
    function resetGame() {
        score = 0; lives = 3; level = 1; powerUp = 0; combo = 0; comboT = 0; kStreak = 0; comboMilestone = 0; dashCD = 0; dashAct = 0; slowMo = 0; freeze = 0; warnT = 0; weaponType = 0; weaponAmmo = 0; ship = mkShip();
    ...
    function update() {
        if (paused) return; if (freeze > 0) { freeze--; return } tick++; updGlitch(); updStars(); updDebris();
        if (state === "play") {
            updShip(); updBullets(); updEnemies(); updEB(); updBoss(); updDrops(); updSparks(); waveT++;
    ...
    function render() {
        if (!ctx) return; ctx.save(); if (shake > 0) ctx.translate(rng(-shake, shake) * dpr, rng(-shake, shake) * dpr);
        ...
        if (state === "won") {
            drawSparks(ctx); ctx.fillStyle = "rgba(8,4,0,.85)"; ctx.fillRect(0, 0, W, H);
            ctx.textAlign = "center"; ctx.fillStyle = "#ffcc44"; ctx.font = _f28b; ctx.fillText("▲ SIGNAL CLEARED ▲", W / 2, H * .3);
            ctx.font = _f42b; ctx.fillText(score, W / 2, H * .43);
            ctx.font = _f11; ctx.fillStyle = "#ffaa33"; ctx.fillText("FINAL SCORE", W / 2, H * .5);
    ...
    function resize() {
        var p = cvs.parentElement; if (!p) return; dpr = mn(window.devicePixelRatio || 1, 2); var rw = p.clientWidth, rh = p.clientHeight; W = F(rw * dpr); H = F(rh * dpr); cvs.width = W; cvs.height = H; cvs.style.width = rw + "px"; cvs.style.height = rh + "px";
        _f7 = F(7 * dpr) + "px monospace"; _f8 = F(8 * dpr) + "px monospace"; _f9 = F(9 * dpr) + "px monospace"; _f9b = "bold " + F(9 * dpr) + "px monospace"; _f11 = F(11 * dpr) + "px monospace"; _f11b = "bold " + F(11 * dpr) + "px monospace"; _f14b = "bold " + F(14 * dpr) + "px monospace"; _f16b = "bold " + F(16 * dpr) + "px monospace"; _f22b = "bold " + F(22 * dpr) + "px monospace"; _f28b = "bold " + F(28 * dpr) + "px monospace"; _f42b = "bold " + F(42 * dpr) + "px monospace";
        initStars(); initDebris(); if (ship) { ship.x = cl(ship.x, ship.w / 2, W - ship.w / 2); ship.y = H - 80 * dpr; ship.w = 42 * dpr; ship.h = 46 * dpr; ship.spd = 6.5 * dpr }
    }
    ...
    return {
        init: function (c, opts) {
            cvs = c; ctx = cvs.getContext("2d", { alpha: false }); opts = opts || {}; initPools(); resize();
