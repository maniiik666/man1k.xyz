...
var tk=0,scanPos=0;
function efx(){tk++;
scanPos=(scanPos+2.8)%(root.offsetHeight||600);scanH.style.top=scanPos+"px";
...
requestAnimationFrame(efx);
var sc=document.getElementById("scanlines");if(sc){var sP=0;(function sd(){sP+=.006;sc.style.backgroundSize="100% "+(3+Math.sin(sP)*.5).toFixed(2)+"px";sc.style.opacity=(.35+Math.sin(sP*.7)*.08).toFixed(2);requestAnimationFrame(sd)})()}
