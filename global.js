/*! MAN1K JS v2.1 | © 2026 Yaroslav Boruk | legal@man1k.xyz */
(function (W, D, L, N) {
  'use strict';
  ...
  var Perf = (function () {
    ...
    var preconnect = function () {
      ...
      var dns = ['soundcloud.com', 'bandsintown.com'];
        ...
        if (pb && pb.u > Date.now()) {
          var showBlocked = function () {
            D.body.innerHTML = '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;background:#000;color:#f00;font-family:monospace;font-size:1.25rem;gap:1rem" role="alert">' +
              '<span>BLOCKED</span>' +
              '<a href="mailto:legal@man1k.xyz" style="color:#f00;font-size:0.9rem">legal@man1k.xyz</a>' +
              '</div>'
          };
  ...
  var Widgets = (function () {
    ...
    var init = function () {
      hint('dns-prefetch', ['bandsintown.com', 'soundcloud.com']);
  ...
  var Guard = (function () {
    if (isBot) return { log: function () { } };
    var warned = false, iv = null;
    var show = function () {
      if (warned) return; warned = true;
      if (iv) { try { clearInterval(iv) } catch (e) { } iv = null }
      if (/\/4\d\d/.test(p)) return;
      try {
        W.console.clear && W.console.clear();
        W.console.log('%c\u00a9 2026 MAN1K \u2014 man1k.xyz', 'color:#d42020;font-family:monospace;font-size:14px;font-weight:bold');
        W.console.log('%cAll source code \u00a9 2026 Yaroslav Boruk (MAN1K). All rights reserved.\nUnauthorized reproduction is monitored \u2192 legal@man1k.xyz', 'color:#555;font-family:monospace;font-size:11px');
      } catch (e) { }
    };
    try {
      iv = setInterval(function () {
        var el = new Image();
        Object.defineProperty(el, 'id', { get: show, configurable: true });
        W.console.log('%c', 'font-size:0;line-height:0', el);
      }, 2500);
    } catch (e) { }
    return { log: show }
  })();
  var GA = (function () {
    ...
    try { gtag('set', 'user_properties', { site_version: C.v }) } catch (ex) { }
    return {
  ...
  var Analytics = (function () {
    if (isBot) return { init: function () { } };
    var setup = function () {
      // Scroll depth
      var depths = [25, 50, 75, 90], dFired = {}, sTick = false;
      W.addEventListener('scroll', function () {
        ...
        requestAnimationFrame(function () {
          ...
          if (dH <= 0) return;
          var pct = Math.round(W.scrollY / dH * 100);
          for (var i = 0; i < depths.length; i++) {
            if (pct >= depths[i] && !dFired[depths[i]]) { dFired[depths[i]] = 1; GA.e('scroll_depth', { depth: depths[i] }) }
          }
        });
      }, { passive: true });
      // Time on page
      var tMs = [30, 60, 120, 300], tF = {}, tS = Date.now();
      setInterval(function () {
        var s = ((Date.now() - tS) / 1000) | 0;
        for (var i = 0; i < tMs.length; i++) {
          if (s >= tMs[i] && !tF[tMs[i]]) { tF[tMs[i]] = 1; GA.e('time_on_page', { seconds: tMs[i] }) }
        }
      }, 10000);
      // Link clicks: outbound, platforms, file downloads, email
      D.addEventListener('click', function (e) {
        var a = e.target.closest('a[href]');
        if (!a) return;
        var href = a.href || '', host = a.hostname || '';
        if (href.indexOf('mailto:') === 0) { GA.e('email_click', { link_text: (a.textContent || '').trim().slice(0, 60), page: p }); return }
        if (/\.(pdf|zip|mp3|wav|flac)$/i.test(href)) { GA.e('file_download', { file_name: href.split('/').pop(), page: p }); return }
        if (!host || host === C.domain || host === 'www.' + C.domain) return;
        var txt = (a.textContent || a.getAttribute('aria-label') || '').trim().slice(0, 60);
        GA.e('outbound_click', { outbound_domain: host, link_text: txt, page: p });
        var pl = { spotify: 'spotify.com', bandcamp: 'bandcamp.com', soundcloud: 'soundcloud.com', youtube: 'youtube.com', 'apple-music': 'music.apple.com', instagram: 'instagram.com', tiktok: 'tiktok.com', bandsintown: 'bandsintown.com' };
        for (var k in pl) { if (host.indexOf(pl[k]) !== -1) { GA.e('platform_click', { platform: k, link_text: txt }); break } }
      }, { passive: true });
      // Content copy
      D.addEventListener('copy', function () { GA.e('content_copy', { page: p }) }, { passive: true });
      // Visitor context (once per session)
      try {
        var vc = 0; try { vc = parseInt(W.localStorage.getItem('_m1k_v1') || 0) || 0 } catch (e2) { }
        var tz = ''; try { tz = Intl.DateTimeFormat().resolvedOptions().timeZone } catch (e3) { }
        GA.e('visitor_context', { visitor_type: vc > 1 ? 'returning' : 'new', user_language: N.language || 'unknown', screen_category: W.innerWidth >= 1200 ? 'desktop' : W.innerWidth >= 768 ? 'tablet' : 'mobile', timezone: tz || 'unknown' });
      } catch (e) { }
      ...
      if (ps === 'subscribe') {
        ...
        if (form) {
          form.addEventListener('submit', function () { GA.e('newsletter_submit', {}) }, { once: true });
          var ei = form.querySelector('input[type="email"]');
          if (ei) ei.addEventListener('focus', function () { GA.e('newsletter_form_focus', {}) }, { once: true });
        }
      }
      if (ps === 'lives') {
        D.addEventListener('click', function (e2) {
          if (e2.target.closest('.bit-button,[class*="bit-rsvp"],[class*="bit-ticket"]')) GA.e('event_rsvp_click', {});
        }, { passive: true });
      }
      if (ps === 'contact') GA.e('contact_page_view', {});
      if (ps === 'press' || ps === 'press/faq') {
        D.addEventListener('click', function (e2) {
          var item = e2.target.closest('.faq-item');
          if (item) { var q = item.querySelector('.faq-q'); if (q) GA.e('faq_expand', { question: q.textContent.trim().slice(0, 80) }) }
        }, { passive: true });
      }
    };
    return { init: setup };
  ...
  W.MAN1K = { v: C.v, Widgets: Widgets, GA: GA, Glitch: Glitch, RL: RL };
